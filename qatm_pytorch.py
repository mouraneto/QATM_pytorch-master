...
def plot_result(image_raw, boxes, show=False, save_name=None, color=(255, 0, 0)):
    ...
    for box in boxes:
        ...
        # text = "teste"
        # text_position = (box[0][0], box[1][1] + 20)
        # cv2.putText(d_img, text, text_position, cv2.FONT_HERSHEY_COMPLEX, 0.8, color=color)
    if show:
        plt.imshow(d_img[:,:,::-1])
    if save_name:
...
def plot_result_multi(image_raw, boxes, indices, show=False, save_name=None, color_list=None):
    ...
    if color_list is None:
        # Supondo que 'indices' seja uma série ou array com valores numéricos
        # Gera uma lista de cores com base no número de índices
        color_list = color_palette("hls", indices.max()+1)

        # Converte as cores da paleta para valores inteiros no formato RGB (0-255)
    ...
    for i in range(len(indices)):
        ...
        box = boxes[i][None, :, :].copy()
        color = color_list[indices[i]]
        d_img = plot_result(d_img, box, color=color)
        text = f"caixa{indices[i]}"
        itens_lista.append(text)
        x, y  = box[0][0][0], box[0][0][1]
        text_position = (x, y - 10)
        cv2.putText(d_img, text, text_position, cv2.FONT_HERSHEY_DUPLEX, 0.5, color=color)
    if show:
        plt.imshow(d_img[:,:,::-1])
    if save_name:
        cv2.imwrite(save_name, d_img)
